...
import engine5 from '@src/libs/slot-engines/engine5'
  ...
  async run () {
    try {
      const {
        ...
        args: { Line, LineBet, isBuyFreeSpin, RoundType, isBuyHoldAndSpinner }
      } = this
      ...
      engine5.init({ clientSeed, serverSeed, betAmount, isBuyFreeSpin, isBuyHoldAndSpinner })
      ...
      const response = {
        ...
        Next: {
          ...((isFreeGameTriggered || isHoldAndSpinnerTriggered)
            ? {
                Type: isFreeGameTriggered ? BET_TYPES_CODE.FREE_SPIN : BET_TYPES_CODE.HOLD_AND_SPIN,
                ...(isFreeGameTriggered
                  ? {
                      FreeSpin: {
                        Next: 1,
      ...
      const slotGameBet = await SlotGameBetModel.create({
        ...
        result: (isFreeGameTriggered || isHoldAndSpinnerTriggered) ? null : (winningAmount > 0 ? BET_RESULT.WON : BET_RESULT.LOST),
        currencyId,
        roundType: RoundType,
        freeSpinWinningAmount: 0,
        baseSpinWinningCombination: baseHoldAndSpinnerResult,
        freeSpinWinningCombination: freeSpinsGameResult,
      ...
      if (debitResponse.status === OPERATOR_RESPONSE_CODES.SUCCESS) {
        ...
        if (!(isFreeGameTriggered || isHoldAndSpinnerTriggered)) {
