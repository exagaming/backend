 ...
 *
 * @export
 * @class PlaceTumbleInFreeSpinBet
 * @extends {ServiceBase}
 */
export class PlaceTumbleInFreeSpinBet extends ServiceBase {
  async run () {
    try {
      const {
        context: {
      ...
      if (Type !== BET_TYPES_CODE.TUMBLE_IN_FREE_SPIN) throw messages.INVALID_TYPE
      ...
      if (previousSpinBetState.next.Type !== BET_TYPES_CODE.TUMBLE_IN_FREE_SPIN) throw messages.INVALID_TYPE
      ...
      let AccWin = plus(currentTumblePayout, previousSpinBetState.current.AccWin)
      ...
      if (!isNextTumble) {
        ...
        if (currentFreeSpinDetail.isAnyMultiplierPresent) {
          // INFO: We calculate the whole multiplied AccWin amount and then we deduct AccWin from it to get remaining amount
          const AccWinMultiplier = times(AccWin, (+currentFreeSpinDetail.globalMultiplier))
          const remainingGlobalMultiplierAmount = minus(AccWinMultiplier, AccWin)
          currentSlotGameBet.freeSpinWinningAmount = plus((+currentSlotGameBet.freeSpinWinningAmount), remainingGlobalMultiplierAmount)
          AccWin = AccWinMultiplier
        }
      }
