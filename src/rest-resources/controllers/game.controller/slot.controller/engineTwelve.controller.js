...
import { PreloadService } from '@src/services/game/slot/engine12/preload.service'
import { TransactService } from '@src/services/game/slot/engine12/transact.service'

export class EngineTwelveController {
  /**
  ...
  static async preload (req, res, next) {
    try {
      const result = await PreloadService.execute(req.body, req.context)
      sendResponse({ req, res, next }, result)
    } catch (error) {
      next(error)
    }
